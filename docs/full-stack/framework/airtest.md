...
# start_gesture() => 滑动手势。可用于解锁等复杂手势操作
poco = Poco(...)
ui1 = poco('xxx')
ui2 = poco('yyy')
...
# NOTE：wait_for_appearance()和wait_for_disappearance()；这两个API可以帮助我们等待页面上 某1个UI 出现或者消失，等待的超时时间 timeout 默认为120秒，如果在超时时长之内元素没有出现或者消失的话，会报 PocoTargetTimeout 的错误
...
# NOTE：生成Airtest报告需要依赖脚本运行的log内容，所以我们在生成报告之前，需要保存脚本运行的log内容，然后生成报告时再指定参数让它去找对应的log内容
...
### 场景实践
代码demo: pytest + airtest + poco
脚本名称：test_andorid_poco.py
设备类型：荣耀Waterplay
Android版本：7.0
测试APP：华为视频
测试场景：获取华为视频电影Tab下的全部电影名称
运行命令：pytest test/app/test_andorid_poco.py -s
```
import os
import pytest
import logging
...
# 全局配置每个操作之间的延迟时间
ST.OPDELAY = 1  # 操作之间间隔太短可能导致异常 => requests.exceptions.ReadTimeout: HTTPConnectionPool(host='127.0.0.1', port=16583): Read timed out. (read timeout=0.3)
...
- 全局变量：https://airtest.doc.io.netease.com/IDEdocs/airtest_framework/7_script_settings/